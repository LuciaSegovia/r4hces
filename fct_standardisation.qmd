...
This document provide an overview of the steps required to the standardisation of FCTs from diverse sources and harmonisation for generating household-specific Nutrient Conversion Tables (NCTs). More details about the framework used and the cleaned data that can be found [in this repository](), and its documentation.
...
![](images/Workflow_v6.0.0.jpg){#fig-framework}

## Identifying and obtaining the food composition data
...
Additionally, we recommend using the "FAO/INFOODS Evaluation framework to assess the quality of published food composition tables and databases - User guide" for evaluating the quality of the FCT that would be used (@charrondiereFAOINFOODSEvaluation2023).

Once the FCTs are selected, if they have not been previously standardised previously (See this repository), then they should be standardised so they can be used and combined with other FCTs. 
...
After identiying and ensurign that we have the right to use/ standardise the data for our study, we can move to R(Studio) to start the importing and standardisation of the new FCT. 
...
and identified using brackets ("[]") around the component values (`CARTEB or [CARTEBEQ]`). However, because we are aiming to have one variable per column, we need to divide the combined variables into two (or more) columns, as such the Beta-carote variable was separated into two independent columns (`CARTEBmg`, `CARTBEQmg`). Note that when we are separating these two food components into the new variable, there are several things that we need to check: we need to check whether they are present in the original FCT, otherwise we may be overwriting values. 
  ...
  2) The variable is present: only overwrite if the values are missing in the original column.
...
Food components definition and re-naming is a key step to be able to combine multiple data sources. This is because each FCT has its own variable names, including for similar food components. Some
